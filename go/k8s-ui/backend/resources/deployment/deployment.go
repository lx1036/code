...
func CreateOrUpdateDeployment(client *kubernetes.Clientset, deployment *v1beta1.Deployment) (*v1beta1.Deployment, error) {
	old, err := client.AppsV1beta1().Deployments(deployment.Namespace).Get(deployment.Name, metaV1.GetOptions{})
	if err != nil {
	...
	return client.AppsV1beta1().Deployments(deployment.Namespace).Update(old)
}
