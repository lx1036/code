...
创建nginx-app的deployment ，使用镜像为nginx:1.24.0 ,修改镜像为1.11.3-alpine，并记录升级，再使用回滚，将镜像回滚至nginx:1.11.0-alpine

kubectl run nginx-app --image=nginx:1.24.0
kubectl set image deployment nginx-app --image=nginx:1.24.0
