...
const MetricsNamespace = "php-fpm"
...
func getDataFastcgi(endpoint *url.URL) ([]byte, error) {
	path := endpoint.Path
	host := endpoint.Host
	...
	fcgi, err := fcgiClient.Dial(endpoint.Scheme, host)
	if err != nil {
...
func (exporter *Exporter) newCollector() *collector {
	return &collector{
		exporter:           exporter,
