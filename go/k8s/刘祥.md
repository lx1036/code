...
fusefs 项目的三个模块：
fuse-client 进程置于 fusefs-CSI Pod 里，为每个 K8S 节点上的每个 pv(可以被多个 pod 挂载) 启动一个 fuse-client 进程来实现 fuse 挂载，该进程实现了 linux fuse vfs 的接口；
master 作为控制平面一般部署3节点，使用 raft 保证数据强一致性，最主要的是提供相关 volume api 供 fusefs-csi 调用，以及 meta node 注册 api，
当用户创建 volume 时，会根据 inode id 范围选择3个 meta partition，并每一个 meta partition 从注册的所有 meta-partition 节点中选择 3 个形成一个 raft；
meta-partition 节点可以无限扩展，一个用户 volume 默认分割成三个 partition，使用 multi-raft 保证数据强一致性，每个 meta-partition 存储了该 volume 的部分文件 inode 和 dentry，
供 fuse-client 进程读写文件时调用。fuse-client 和 meta-partition 进程为了性能使用 TCP 通信；
...
(2)主要参与开发 master 模块。主要参与开发 meta node 注册相关 api，以及 create/delete volume 等相关 api 被 fusefs-csi 调用，
...
内容：独立开发整个 CSI 项目。
CSI 实现了 IdentityServer service ，借助 node-driver-registrar sidecar 容器向 kubelet 里注册当前 CSI driver；
...
等 CSI Pod 重建完成，业务 Pod 可以继续读写数据而不用重建，而在该方案之前，却是业务 Pod 一直不可用只能重启才可以(生产环境不现实)，新方案大大减缓了业务 Pod 使用 PVC 的痛点。
