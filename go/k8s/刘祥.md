...
背景：目前公司内部生产 K8S 部署的业务 Pod 绝大多数都是在线业务 pod，白天吃资源大晚上吃资源少。而随着大数据和人工智能离线业务 Pod 开始上 K8S 后，
...
更新在离线 Pod 的容器是周期性去计算更新的。总之，通过使用 cpuset 物理核完全隔离来避免超线程干扰。但是这种通过周期性更新方式响应比较慢，
不能很好解决在线 Pod 突然暴增需要减少分配给离线 Pod 的资源，而且这种方式也没有更好压榨机器资源。并且，kubelet cpu manager 只能是 none policy，
不能是 static。
...
更高优先级后，当在线业务 Pod 突然暴增需要更多资源时，GroupIdentity 可以快速响应暂时压制离线业务 Pod。
...
最后，由于和阿里有战略合作，有幸和阿里内部人员共同验证过阿里混合部署的 K8S 套件，做过 POC 验证测试，包括 CPU/Memory/Network 隔离等等，最重要的对
在线业务 Pod 的干扰率小于 5% 以内，的确强悍。

相关经验：对 kubelet CPU 管理和 CPU NUMA 机制都比较了解。
...
的内存申请，所以需要 kernel 支持内存异步回收。阿里内核通过设置 cgroup memory.wmark_ratio(0-100) 值在内核层支持异步回收，当内存
...
整个过程无需 BGP 去宣告路由，且不存在不同规格 Node 的 PodIP 资源浪费问题。且和 VPC 网络完全融合，容器团队无需再和网络部门沟通 BGP 网络问题，
VPC 网络问题都交给虚拟化团队去负责。

针对容器上虚机场景，需要开发 CNI Bin 组件和 IPAM Daemon 组件。
(1) Bin 组件完全实现 CNI 标准接口函数，。
grpc 调用 IPAM Daemon 组件获取 PodIP，并创建 ipvlan 网卡 eth0 置于 Pod 内，该 eth0 的 parent 网卡是弹性网卡，采用 ipvlan L2 mode，即弹性网卡作为二层网桥。
...
比如创建挂载弹性网卡、申请 IP 等 API 一般都是十秒左右才会生效，是很慢的，如果创建销毁 Pod 时同步去调用 VM API，会导致配置 Pod 网络资源很慢，所以需要缓存并异步，提前申请 IP 资源，实现资源池化。
...
并且，把所有 Pod 已经在使用 IP 资源存入 IPPool inuse 中，同时也存入 boltdb 存储并落盘，这样重启后可以从 boltdb 文件 restore 回 inuse 和 idle 中。
最后，这样就实现了弹性网卡辅助 IP 的资源池化，当空闲 IP 小于资源池最小水位时则自动补足 IP 资源，当空闲 IP 大于资源池最大水位时则自动释放 IP 资源，保障了资源的高效利用和分配效率。

难点：serviceCidr；(2)IP 资源池化架构设计，代码实现有难度，且复杂。

不足：还没有实现基于 pod 的 ingress/egress 网络带宽限制 bandwidth。固定 podIP 功能。
...
# 个人描述
在项目开发中，顺便对社区相关开源项目做过一些 PR，比如，，不过都是一些小 PR。
