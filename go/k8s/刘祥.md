...
然后该容器作为 csi client 调用 CreateVolume/DeleteVolume/ExpandVolume 等 ControllerServer service 接口，在这些接口中会调用 fusefs master 组件提供的 HTTP API；
...
这里利用了 kubelet 的块存储设计机制：在调用相同 pod 的多个副本时，第一个 pod 会调用 NodeStageVolume 和 NodePublishVolume，但是随后的 pod 副本不会调用 NodeStageVolume，
而只会调用 NodePublishVolume。而社区内很多 csi 没有使用 NodeStageVolume(比如 juicefs-csi 等)，而是在 NodePublishVolume 里去做 mount xxx/globalmount 挂载点然后 bind mount 
