...
开发 fusefs 项目：fusefs 项目包含 fuse-client、master-cluster(raft) 和 meta-partition-cluster(multi-raft) 三个模块。
* 参与开发 fuse-client 模块，使用 fuse 实现本地化直接读写文件，数据存储在远程 S3 上。并且修改第三方 fuse 包，使其支持 macOS 系统，使得可以 mac 本地运行。
* 参与开发 master-cluster 模块，

开发 fusefs-csi 项目：为分布式文件存储实现一个 K8s CSI，方便业务 Pod 动态挂载 fusefs PVC
