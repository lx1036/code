...
(7.2) pod 调度器 scheduler plugin 自定义开发
内容：人工智能训练作业需要调度器批量调度 pod，而目前 K8S scheduler 的 plugins 是单个 pod 调度，需要开发一个 batch-pods plugin 来支持一组 pod 批量调度，
...
内容：混合部署时，需要根据 Node 上给离线 Spark pod 作业分配的自定义资源 colocation/cpu 和 colocation/memory 来调度 colocation-spark plugin。

难点：这里主要是 pod resource 内有自定义资源，调度器在 PreFilter/Filter plugin 阶段读取 node allocatable 中 colocation/cpu 和 colocation/memory 来进行过滤，
而 node allocatable 中 colocation/cpu 和 colocation/memory 是由另一个 webhook 组件读取 metrics-server 获取 node 真实 cpu/memory 资源，按照一定比例计算后，
把最终值 patch 到 node allocatable status 中的。plugin 难点不大，比较简单。
...
etcd-operator etcd 中间件上 K8S 项目：对 etcd 机制和内部原理比较了解
大数据上 K8S 项目：涉及到的技术栈 spark-operator 和 volcano
