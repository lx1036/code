...
import (
	...
	"k8s-lx1036/k8s/storage/raft/raft"
...
func LeaderCheck(handler func(http.ResponseWriter, *http.Request), server *raft.Server) func(w http.ResponseWriter, r *http.Request) {
...
func NewServer(config *raft.Config) (*Server, error) {
	stateMachine := &StateMachine{
		data: map[string]string{},
	}
	server, err := raft.NewServer(config, stateMachine, config.Nodes, config.Node)
	if err != nil {
		return nil, err
	}
