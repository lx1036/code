...
import (
	...
	versioned "k8s-lx1036/k8s/bigdata/spark-on-k8s/spark-operator/pkg/client/clientset/versioned"
	internalinterfaces "k8s-lx1036/k8s/bigdata/spark-on-k8s/spark-operator/pkg/client/informers/externalversions/internalinterfaces"
	v1 "k8s-lx1036/k8s/bigdata/spark-on-k8s/spark-operator/pkg/client/listers/sparkoperator.k9s.io/v1"
	time "time"
...
type SparkApplicationInformer interface {
	...
	Lister() v1.SparkApplicationLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.SparkoperatorV1().SparkApplications(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.SparkoperatorV1().SparkApplications(namespace).Watch(context.TODO(), options)
			},
		},
		&sparkoperatork9siov1.SparkApplication{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *sparkApplicationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&sparkoperatork9siov1.SparkApplication{}, f.defaultInformer)
}

func (f *sparkApplicationInformer) Lister() v1.SparkApplicationLister {
	return v1.NewSparkApplicationLister(f.Informer().GetIndexer())
}
