...
main()
{
	...
	BPF_PROG_ID=$(ns1_exec tc filter show dev "${TEST_IF}" ingress | get_prog_id)
...
#	ns1_exec tc qdisc del dev "${TEST_IF}" clsact
  ...
  sleep 5
	...
	printf "Testing XDP..."
	ns1_exec ip link set "${TEST_IF}" xdp object "${BPF_PROG_OBJ}" section "${XDP_SECTION}"
...
TEST_IF=lo
MAX_PING_TRIES=5
BPF_PROG_OBJ="${DIR}/tcp_syncookie.o"
