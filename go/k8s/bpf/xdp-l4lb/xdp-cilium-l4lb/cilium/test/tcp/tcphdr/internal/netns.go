...
func openNetNS(path, bpfFsPath string) (ns.NetNS, string, error) {
	...
	dir := fmt.Sprintf("%d_tcp_hdr", stat.Ino)    // inode(/proc/self/ns/net)
	return ns, filepath.Join(bpfFsPath, dir), nil // /sys/fs/bpf/{inode}_tcp_hdr
}

func programPath(base string) string {
