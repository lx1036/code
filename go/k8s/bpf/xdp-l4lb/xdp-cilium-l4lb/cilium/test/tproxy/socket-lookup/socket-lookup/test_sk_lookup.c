...
int dispatcher(struct bpf_sk_lookup *ctx) {
    /* /32 and /128 */
    struct ip laddr_full = {}; // INFO: 注意这里的赋值
    if (ctx->family == AF_INET) { // 为何这里这么赋值???
    ...
    struct addr key = {
            .prefixlen = (sizeof(struct addr) - 4) * 8, // 这里为何这么定义???
    ...
    metrics->lookups++;
    struct bpf_sock *sk = bpf_map_lookup_elem(&sockets, &bind->id);
    if (!sk) {
