...
#ifndef XDP_CILIUM_L4LB_DROP_H
#define XDP_CILIUM_L4LB_DROP_H
...
static __always_inline int send_drop_notify_error(struct __ctx_buff *ctx, __u32 src, int error, int exitcode, __u8 direction) {
    return send_drop_notify(ctx, src, 0, 0, error, exitcode, direction);
}

#endif //XDP_CILIUM_L4LB_DROP_H
