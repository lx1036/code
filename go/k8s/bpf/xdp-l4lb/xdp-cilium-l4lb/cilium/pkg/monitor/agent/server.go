...
// buildServer opens a listener socket at path. It exits with logging on all errors.
// server on /var/run/cilium/monitor1_2.sock
func buildServer(path string) (*net.UnixListener, error) {
	...
	srv, err := net.ListenUnix("unix", addr)
	if err != nil {
