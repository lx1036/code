...
for example, the server name requested through SNI or protocols advertised in ALPN.
*/
...
static ngx_int_t ngx_stream_ssl_preread_init(ngx_conf_t *cf) {
    ...
    h = ngx_array_push(&cmcf->phases[NGX_STREAM_PREREAD_PHASE].handlers); // NGX_STREAM_PREREAD_PHASE.handler -> ngx_stream_ssl_preread_handler()
    if (h == NULL) {
        return NGX_ERROR;
    }
