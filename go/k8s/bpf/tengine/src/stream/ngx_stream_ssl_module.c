...
static ngx_int_t ngx_stream_ssl_init(ngx_conf_t *cf) {
    ...
    *h = ngx_stream_ssl_handler; // NGX_STREAM_SSL_PHASE.handler -> ngx_stream_ssl_handler()

#if (T_NGX_HAVE_DTLS)
    cmcf = ngx_stream_conf_get_module_main_conf(cf, ngx_stream_core_module);
    ls = cmcf->listen.elts;
    for (i = 0; i < cmcf->listen.nelts; i++) {
        if (ls[i].ssl) { // ngx_stream_core_listen() 里 "ssl" 开关开启
            sctx = ls[i].ctx;
            ...
            if (ls[i].type == SOCK_DGRAM) {
                if (!(sscf->protocols & NGX_SSL_DTLSv1 || sscf->protocols & NGX_SSL_DTLSv1_2)) {
                    ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
...
static ngx_int_t ngx_stream_ssl_handler(ngx_stream_session_t *s) {
    ...
    if (c->ssl == NULL) {
        ...
        rv = ngx_stream_ssl_init_connection(&sslcf->ssl, c); // 在这里会返回 Server Hello 报文, Change Cipher Spec
        if (rv != NGX_OK) {
            return rv;
        }
    }
