...
func forwardL2(verbose bool, inLink netlink.Link, inLinkQueueID int, inLinkDst net.HardwareAddr, outLink netlink.Link, outLinkQueueID int, outLinkDst net.HardwareAddr) {
	...
	inProg, err := xdp_socket.NewProgram(inLinkQueueID + 1)
	if err != nil {
	...
	inXsk, err := xdp_socket.NewSocket(inLink.Attrs().Index, inLinkQueueID, nil)
	if err != nil {
	...
	outXsk, err := xdp_socket.NewSocket(outLink.Attrs().Index, outLinkQueueID, nil)
	if err != nil {
...
func forwardFrames(input *xdp_socket.Socket, output *xdp_socket.Socket, dstMac net.HardwareAddr) (numBytes uint64, numFrames uint64) {
...
func replaceDstMac(xsk *xdp_socket.Socket, descs []xdp_socket.Desc, dstMac net.HardwareAddr) {
	for _, d := range descs {
