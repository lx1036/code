...
CALICOCTL_IMAGE=calico/ctl:v3.12.0
docker pull ${CALICOCTL_IMAGE}
docker create --name calicoctl-create ${CALICOCTL_IMAGE}
sudo docker cp calicoctl-create:/calicoctl /usr/local/bin/calicoctl
sudo chmod +x /usr/local/bin/calicoctl
docker rm calicoctl-create
...
# 没有 DATASTORE_TYPE: https://github.com/projectcalico/calico/issues/1656#issuecomment-362896508
sudo sh -c "cat > /etc/calico/calico2.env" << EOF
NODENAME=$(hostname)
NO_DEFAULT_POOLS=false
IP=""
IP6=""
...
WantedBy=multi-user.target
EOF
# sudo journalctl -u calico-libnetwork-plugin.service 查看启动错误日志
