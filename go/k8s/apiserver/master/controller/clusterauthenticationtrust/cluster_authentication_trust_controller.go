...
const (
	...
	configMapName      = "extension-apiserver-authentication-fake" // 这里修改了 cm name，原来是 extension-apiserver-authentication
)
...
func NewClusterAuthenticationTrustController(requiredAuthenticationData ClusterAuthenticationInfo, kubeClient kubernetes.Interface) *Controller {
	...
	kubeSystemConfigMapInformer := corev1informers.NewConfigMapInformer(kubeClient, configMapNamespace, 12*time.Hour, cache.Indexers{
